...
var_choices = ["mass", "density", "temp", "metallicity", "H2_fraction", "tan_vel", "rad_vel_sign", "rad_vel", "LW", "sp_gas", "sp_dm", "rms_vel", "closest_galaxy", "overdensity", "radial_mass_flux_sign",
               "radial_mass_flux", "av_mass", "av_dMdt_sign", "av_dMdt", "av_dMdz_sign", "av_dMdz", "t1", "stellar_mass"]
...
parser.add_argument("-c", "--vars", type=str, nargs=2, choices=var_choices, default=None,
                    dest="vars", help="variables defining the 2D subspace")
...
v1, v2 = list(sorted(args.vars, key=lambda v: var_choices.index(v)))

outfile = f"{args.outfolder}results_{v1}_{v2}"
...
if (args.kernel == "poly"):
    outfile = args.outfolder + "results_%s%s_logC%s_logw%s.npy"%(args.kernel, args.degree, args.logC, args.logw)
else:
    outfile = args.outfolder + "results_%s_logC%s_logw%s.npy"%(args.kernel, args.logC, args.logw)
